#include <Package/Package.h>

#include <Logging/Logging.h>

#include <Animation/ImportAnimation.h>
#include <Audio/ImportAudio.h>
#include <Light/ImportLight.h>
#include <Material/ImportMaterial.h>
#include <Mesh/ImportMesh.h>
#include <ImportScene.h>
#include <Texture/ImportTexture.h>

#include <Utilities/Hash.h>

BEGINNAMESPACE

using namespace Resource;

Package::Package() : m_Dirty(false), m_RecentTimestamp(0), m_ResourceCount(0)
{}

Package::Package(const String & pkgName, const PackageManager& mgr) : m_Dirty(false), m_packageName(pkgName), m_RecentTimestamp(0), m_ResourceCount(0)
{	
	m_AnimationFolder = mgr.getAnimationRoot() / m_packageName;
	m_AudioFolder = mgr.getAudioRoot() / m_packageName;
	m_LightFolder = mgr.getLightRoot() / m_packageName;
	m_MaterialFolder = mgr.getMaterialRoot() / m_packageName;
	m_MeshFolder = mgr.getMeshRoot() / m_packageName;
	m_SceneFolder = mgr.getSceneRoot() / m_packageName;
	m_TextureFolder = mgr.getTextureRoot() / m_packageName;
	m_PackagePath = mgr.getPackageRoot() / (m_packageName + ".pkg");
}

uint32 Package::update_scene()
{
	uint32 max_currentTimeStamp = m_RecentTimestamp;

	if (!is_directory(m_SceneFolder)) return max_currentTimeStamp; //The Package has no folder in the scene-folder

	for (auto& file_entry : directory_iterator(m_SceneFolder)) {
		path file = file_entry.path();
		if (!is_regular_file(file)) continue;

		file_time_type ft = last_write_time(file);
		uint32 currentTimeStamp = std::chrono::system_clock::to_time_t(ft);

		if (alreadyTracked(file)) {
			//This file was processed before: check wheter it has to be updated
			if (currentTimeStamp <= m_RecentTimestamp) continue; //this resource is up to date
		}

		//This file is either out of date or wasn't tracked before
	   //load from file
		if (!Importer::sceneLoad(file, *this, currentTimeStamp)) {
			LOG_ERROR(General, "The scene (%s) load failed.", file.c_str());
			continue;
		}

		setFileTracked(file);

		max_currentTimeStamp = std::max(currentTimeStamp, max_currentTimeStamp);
	}
	return max_currentTimeStamp;
}

bool Package::alreadyTracked(const path & file)
{
	String filename = m_packageName + "/" + file.stem().string();
	uint32 hash = crc32((Byte*)filename.c_str(), filename.length());
	return std::find(m_TrackedFiles.begin(), m_TrackedFiles.end(), hash) != m_TrackedFiles.end();
}

void Package::setFileTracked(const path & file)
{
	String filename = m_packageName + "/" + file.stem().string();
	uint32 hash = crc32((Byte*)filename.c_str(), filename.length());
	m_TrackedFiles.push_back(hash);
}

void Package::attachTracked(uint32 hash)
{
	m_TrackedFiles.push_back(hash);
}

void Package::readHeader(Byte* data, Package::EntryHeader& header, size_type& off) {
	std::memcpy(&header, data + off, sizeof(EntryHeader));
	off += sizeof(EntryHeader);
}

//
/// Load a package from file and inject all objects right into the memory
bool Package::load(const path & filename, const PackageManager& mgr)
{

	m_PackagePath = filename;
	m_packageName = m_PackagePath.stem().string();
	m_AnimationFolder = mgr.getAnimationRoot() / m_packageName;
	m_AudioFolder = mgr.getAudioRoot() / m_packageName;
	m_LightFolder = mgr.getLightRoot() / m_packageName;
	m_MaterialFolder = mgr.getMaterialRoot() / m_packageName;
	m_MeshFolder = mgr.getMeshRoot() / m_packageName;
	m_SceneFolder = mgr.getSceneRoot() / m_packageName;
	m_TextureFolder = mgr.getTextureRoot() / m_packageName;

	//
	/// buffer whole file
	size_type size = file_size(filename);

	String file = filename.string();
	FILE* ifile = fopen(file.c_str(), "r");

	if (!ifile) 
		return false;
	Byte* buffer = new Byte[size];
	fread((ansichar*)buffer, size, 1, ifile );
	fclose(ifile);

	size_type readOffset = 0;

	uint32 trackedFiles = *(uint32*)(buffer + readOffset);
	readOffset += sizeof(uint32);
	for (uint32 i = 0; i < trackedFiles; ++i)
	{
		attachTracked(*(uint32*)(buffer + readOffset));
		readOffset += sizeof(uint32);
	}
	m_ResourceCount = *(uint32*)(buffer + readOffset);
	readOffset += sizeof(uint32);

	uint32 offset = m_ResourceCount * sizeof(EntryHeader) + sizeof(uint32); //The first part of the package contains a library, right after the entries, the resources starts
	for (uint32 res = 0; res < m_ResourceCount; ++res) {
		//read in resource headers
		EntryHeader header = { 0,0,0,0 };
		readHeader(buffer, header, readOffset);

		Byte* resourceMem = buffer + offset;
		offset += header.size;

		Resource::Material* materialLibrary = nullptr; //get the package material library
		Resource::Texture* textureLibrary = nullptr; //get the package material library

		switch (header.type) {
		case ResourceType::Animation:
		{
			Animation* animation = animation::FromBuffer(resourceMem);
			Storage<Animation> entry = { path(), animation, header };
			m_Animations.push_back(entry);
		}
		break;
		case ResourceType::Audio:
		{
			Audio* audio = audio::FromBuffer(resourceMem);
			Storage<Audio> entry = { path(), audio, header };
			m_Audio.push_back(entry);
		}
		break;
		case ResourceType::Light:
		{
			Light* light = (Light*)resourceMem;
			Storage<Light> entry = { path(), light, header };
			m_Lights.push_back(entry);
		}
		break;
		case ResourceType::Material:
		{
			Material* material = material::FromBuffer(resourceMem, textureLibrary);
			Storage<Material> entry = { path(), material, header };
			m_Materials.push_back(entry);
		}
		break;
		case ResourceType::Mesh:
		{
			Mesh* mesh = mesh::FromBuffer(resourceMem, materialLibrary);
			Storage<Mesh> entry = { path(), mesh, header };
			m_Meshes.push_back(entry);
		}
		break;
		case ResourceType::Texture:
		{
			Texture* texture = texture::FromBuffer(resourceMem);
			Storage<Texture> entry = { path(), texture, header };
			m_Textures.push_back(entry);
		}
		break;
		}
	};

	m_Dirty = false; //loaded from .pkg file -> not dirty

	file_time_type ft = last_write_time(file);
	m_RecentTimestamp = std::chrono::system_clock::to_time_t(ft);
	
	return true;
}

void Package::storeHeader(FILE* file, Package::EntryHeader& header) {
	fwrite(&header, sizeof(uint32), 4, file);
}

bool Package::store(const PackageManager& mgr)
{
	if (!m_Dirty) return true;

	FILE* file = fopen(m_PackagePath.string().c_str(), "w+");
		
	if (!file) return false;
	
	Resource::Material* materialLibrary = nullptr; //get the package material library
	Resource::Texture* textureLibrary = nullptr; //get the package material library
	
	uint32 trackedFiles = uint32(m_TrackedFiles.size());
	fwrite(&trackedFiles, sizeof(uint32), 1, file);
	fwrite(m_TrackedFiles.data(), sizeof(uint32), trackedFiles, file);

	fwrite(&m_ResourceCount, sizeof(uint32), 1, file);
	//add a library entry for each resource
	for (auto animation : m_Animations) {
		storeHeader(file, animation.header);
	}
	for (auto audio : m_Audio) {
		storeHeader(file, audio.header);
	}
	for (auto light : m_Lights) {
		storeHeader(file, light.header);
	}
	for (auto material : m_Materials) {
		storeHeader(file, material.header);
	}
	for (auto mesh : m_Meshes) {
		storeHeader(file, mesh.header);
	}
	for (auto texture : m_Textures) {
		storeHeader(file, texture.header);
	}

	std::vector<Byte> buffer;

	//store the actual data right after the library
	for (auto anim : m_Animations) {
		uint32 animationSize = animation::MemorySize(anim.resource);
		buffer.resize(animationSize);
		animation::ToBuffer(anim.resource, buffer.data());
		fwrite(buffer.data(), animationSize, 1, file);
	}
	for (auto aud : m_Audio) {
		uint32 audioSize = audio::MemorySize(aud.resource);
		buffer.resize(audioSize);
		audio::ToBuffer(aud.resource, buffer.data());
		fwrite(buffer.data(), audioSize, 1, file);
	}
	for (auto li : m_Lights) {
		uint32 lightSize = light::MemorySize(li.resource);
		buffer.resize(lightSize);
		light::ToBuffer(li.resource, buffer.data());
		fwrite(buffer.data(), lightSize, 1, file);
	}
	for (auto mat : m_Materials) {
		uint32 materialSize = material::MemorySize(mat.resource);
		buffer.resize(materialSize);
		material::ToBuffer(mat.resource, buffer.data(),textureLibrary);
		fwrite(buffer.data(), materialSize, 1, file);
	}
	for (auto m : m_Meshes) {
		uint32 meshSize = mesh::MemorySize(m.resource);
		buffer.resize(meshSize);
		mesh::ToBuffer(m.resource, buffer.data(), materialLibrary);
		fwrite(buffer.data(), meshSize, 1, file);
	}
	for (auto tex : m_Textures) {
		uint32 textureSize = texture::MemorySize(tex.resource);
		buffer.resize(textureSize);
		texture::ToBuffer(tex.resource, buffer.data());
		fwrite(buffer.data(), textureSize, 1, file);
	}
	
	fclose(file);
	m_Dirty = false;
	
	return true;
}

void Package::update(const PackageManager & mgr)
{
	uint32 timeStamp = m_RecentTimestamp;

	timeStamp = std::max(timeStamp, updater(m_AnimationFolder, m_Animations, &Importer::animationAllFromFile, &animation::MemorySize) );
	timeStamp = std::max(timeStamp, updater(m_AudioFolder, m_Audio, &Importer::audioAllFromFile, &audio::MemorySize) );
	timeStamp = std::max(timeStamp, updater(m_LightFolder, m_Lights, &Importer::lightsAllFromFile, &light::MemorySize) );
	timeStamp = std::max(timeStamp, updater(m_MaterialFolder, m_Materials, &Importer::materialAllFromFile, &material::MemorySize) );
	timeStamp = std::max(timeStamp, updater(m_MeshFolder, m_Meshes, &Importer::meshAllFromFile, &mesh::MemorySize) );
	timeStamp = std::max(timeStamp, updater(m_TextureFolder, m_Textures, &Importer::textureAllFromFile, &texture::MemorySize));
	timeStamp = std::max(timeStamp, update_scene());
	m_RecentTimestamp = timeStamp;
}

void Package::addAnimationFromScene(Animation * animation, const path & sceneFile, uint32 animationSize, uint32 hash, uint32 timeStamp)
{
	m_RecentTimestamp = std::max(m_RecentTimestamp, timeStamp);
	String stem = sceneFile.stem().string();
	EntryHeader header = { ResourceType::Animation, animationSize, hash, timeStamp };

	Storage<Animation> entry = { sceneFile, animation, header };
	m_Animations.push_back(entry);
	++m_ResourceCount;
	m_Dirty = true;
}

void Package::addAudioFromScene(Audio * audio, const path & sceneFile, uint32 audioSize, uint32 hash, uint32 timeStamp)
{
	m_RecentTimestamp = std::max(m_RecentTimestamp, timeStamp);
	String stem = sceneFile.stem().string();
	EntryHeader header = { ResourceType::Audio, audioSize, hash, timeStamp };

	Storage<Audio> entry = { sceneFile, audio, header };
	m_Audio.push_back(entry);
	++m_ResourceCount;
	m_Dirty = true;
}

void Package::addLightFromScene(Light * light, const path & sceneFile, uint32 lightSize, uint32 hash, uint32 timeStamp)
{
	m_RecentTimestamp = std::max(m_RecentTimestamp, timeStamp);
	String stem = sceneFile.stem().string();
	EntryHeader header = { ResourceType::Light, lightSize, hash, timeStamp };

	Storage<Light> entry = { sceneFile, light, header };
	m_Lights.push_back(entry);
	++m_ResourceCount;
	m_Dirty = true;
}

void Package::addMaterialFromScene(Material * material, const path & sceneFile, uint32 materialSize, uint32 hash, uint32 timeStamp)
{
	m_RecentTimestamp = std::max(m_RecentTimestamp, timeStamp);
	String stem = sceneFile.stem().string();
	EntryHeader header = { ResourceType::Light, materialSize, hash, timeStamp };

	Storage<Material> entry = { sceneFile, material, header };
	m_Materials.push_back(entry);
	++m_ResourceCount;
	m_Dirty = true;
}

void Package::addMeshFromScene(Mesh * mesh, const path& sceneFile, uint32 meshSize, uint32 hash, uint32 timeStamp)
{
	m_RecentTimestamp = std::max(m_RecentTimestamp, timeStamp);
	String stem = sceneFile.stem().string();
	EntryHeader header = { ResourceType::Mesh, meshSize, hash, timeStamp };

	Storage<Mesh> entry = { sceneFile, mesh, header };
	m_Meshes.push_back(entry);
	++m_ResourceCount;
	m_Dirty = true;
}

void Package::addTextureFromScene(Texture * texture, const path & sceneFile, uint32 textureSize, uint32 hash, uint32 timeStamp)
{
	m_RecentTimestamp = std::max(m_RecentTimestamp, timeStamp);
	String stem = sceneFile.stem().string();
	EntryHeader header = { ResourceType::Light, textureSize, hash, timeStamp };

	Storage<Texture> entry = { sceneFile, texture, header };
	m_Textures.push_back(entry);
	++m_ResourceCount;
	m_Dirty = true;
}

uint32 Package::getRecentTimeStamp()
{
	return uint32(m_RecentTimestamp);
}


ENDNAMESPACE